...
export function UploadZoneProgress({ patientId }: { patientId: string }) {
    ...
    function addFiles(fileList: FileList | null) {
        ...
        // guardo “file” en cierre sin tipar, no lo incluyo en Row público
        setRows(prev => [...incoming.map(({ file, ...rest }) => rest as Row), ...prev]);
        // subo cada uno
        incoming.forEach(({ file, ...rest }) => startUpload(rest.id, file));
        // limpiar input para volver a elegir lo mismo
    ...
    function startUpload(id: string, file: File) {
        ...
        xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                // refresco lista tras un pequeño delay para que se vea el “100%”
        ...
        // guardo xhr para poder cancelar
        // (truco: cierro sobre id)
        (xhr as any).__rowId = id;
    ...
    return (
        <div className="space-y-3">
            ...
            {/* Lista de subidas en curso / recientes */}
            {rows.length > 0 && (
